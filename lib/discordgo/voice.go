...
func (v *VoiceConnection) handleDAVEBinary(message []byte) {
	...
	case 30:
		...
		if err := dave.HandleExecuteTransition(transitionID); err != nil {
			v.log(LogError, "DAVE execute_transition failed after Welcome: %s", err)
		}
		v.log(LogInformational, "DAVE encryption prepared and activated after Welcome (canEncrypt=%v)", dave.CanEncrypt())
